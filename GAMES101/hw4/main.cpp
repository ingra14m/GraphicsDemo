...
void bezier(const std::vector<cv::Point2f> &control_points, cv::Mat &window, bool MSAA = true) {
    ...
    for (double t = 0.0; t <= 1.0; t += 0.0001) {
        ...
        if(MSAA)
        {
            std::cout << "walk into MSAA" << std::endl;
            // anti-alising
            ...
            float offset_x = x < 0.5f ? -0.5f : 1.5f;
            float offset_y = y < 0.5f ? -0.5f : 1.5f;
            ...
            for (cv::Point2f p:sample_points) {
                ...
                color[1] = std::max((float) color[1], 255 * (distance / base));  // point代表采样点的颜色，base代表距离，distance代表
                //            color[1] = 255;
...
int main() {
    ...
    cv::setMouseCallback("Bezier Curve", mouse_handler, nullptr);  // 设置回调事件，用于处理鼠标点击事件
